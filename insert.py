...
def get_headrs(conn, tbl): #Достаём заголовки стаблиц
...
def getID(conn, t_name): # Добывает идентификатор из выбранной таблицы
...
def create_expr(conn, tbl, data): # создаём список данных, которые подлежат вставке. вход БЕЗ АЙДИШНИКА
    hdrs = get_headrs(conn, tbl)
    res = []
    j = 1
    for row in data:
        lst = {}
...
def insert_into(conn, tbl, data): # Главная функция вставки
    expr = conn.execute(
        insert(tbl),
        create_expr(conn, tbl, data)
        ,)
    conn.commit()
...
def give_sch(conn, st_name):
    stmt = (select(Lesson.start_time, Lesson.end_time, Lesson.cabinet, WeekDays.day_name).join(Lesson).join(
        Study_group).join(Group_stud).join(Student).where(Student.name == st_name))
    answ = conn.execute(stmt)
    result = []
    for row in answ:
        result.append(
